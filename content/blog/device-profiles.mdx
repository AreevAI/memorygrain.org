...
<Callout type="info">
The Lightweight profile's 512-byte limit is not arbitrary. The Vector 1 test grain — a minimal Belief with 9 fields including an `author_did` of 56 characters — is 159 bytes. A Belief with just the required Lightweight fields (type, subject, relation, object, confidence, created_at, namespace) would be roughly 80-100 bytes, depending on string lengths. 512 bytes provides meaningful headroom for longer strings while remaining feasible for microcontrollers with 256 KB or less of total RAM.
</Callout>
...
**Minimal Belief (required fields only):**
A grain like `"user prefers dark mode"` with type, subject, relation, object, confidence, created_at, and namespace. This produces roughly 100-160 bytes, depending on string lengths. The Vector 1 test grain, which includes an `author_did` (56 characters of DID string), is 159 bytes. Fits in all three profiles.

**Annotated Belief (common optional fields):**
...
**Richly annotated Belief (provenance, cross-links):**
...
An Event grain with a `content` field containing a full conversation transcript — say, 10 KB of text. Fits in Standard (32 KB limit) and Extended (1 MB limit), but not Lightweight (512 bytes).
...
- **ESP32**: 520 KB SRAM, hardware SHA-256, WiFi/BLE. The most popular microcontroller for IoT. Can generate Lightweight Belief and Observation grains, hash them, and transmit via MQTT over TLS.
