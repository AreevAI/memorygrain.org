...
The `source_type` of `"imported"` (Section 6.1) indicates this fact was extracted from an external document through automated analysis, not stated by a human or inferred by the agent from patterns. The `derived_from` array links this Belief back to the specific Action grain that performed the document analysis — the provenance chain from knowledge claim to source document.
...
### Actions for legal system interactions
...
Section 10.2 defines which fields can be elided. The `type` field cannot be elided (the receiver must know it is a Belief). The `relation` field cannot be elided (it is core knowledge structure). The `confidence` field cannot be elided (it is essential for trust decisions). But `subject`, `object`, `context`, `structural_tags`, and `provenance_chain` can all be elided.
...
Because every grain in the .mg file is content-addressed, deduplication is automatic. If the same Belief appears in multiple provenance chains, it is stored once. The offset index enables random access even in large evidence packages. And the zstd compression (Section 11.3) reduces file sizes while preserving the ability to decompress and access individual grains.
</Callout>
...
**3. Deposition preparation.** Deposition summaries enter as Events. Key testimony is extracted as Beliefs with `derived_from` pointing to the Event. Contradictions between testimony and documentary evidence are tracked through `related_to` cross-links with `relation_type: "contradicts"` (Section 14.3).
