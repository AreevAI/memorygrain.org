...
```json
{
  ...
  "derived_from": ["<document-analysis-tool-hash>"],
  "structural_tags": ["legal:litigation_hold"]
}
```

The `source_type` of `"imported"` (Section 6.1) indicates this fact was extracted from an external document through automated analysis, not stated by a human or inferred by the agent from patterns. The `derived_from` array links this Fact back to the specific Tool grain that performed the document analysis — the provenance chain from knowledge claim to source document.
...
Deposition summaries, case research notes, and client consultation records are unstructured text — ideal for the Event type (Section 8.2):
...
### Tools for legal system interactions

Every interaction with external legal systems — document review platforms, court filing systems, legal databases — is recorded as a Tool grain:
...
- It is an Event (the `type` is non-elidable)
...
```json
{
  ...
  "provenance_chain": [
    ...
    {
      "source_hash": "<email-review-tool-hash>",
...
A single .mg file can contain all grains relevant to a case — Facts extracted from contracts, Episodes from depositions, Tools documenting the analysis process, and the provenance chains linking them together. The footer checksum (Section 11.5) proves the file has not been tampered with since creation.
...
Grains:  847 Facts (contract analysis, testimony extraction)
         ...
         1,102 Tools (document review, database searches)
...
**1. Document collection.** AI agents ingest contracts, emails, and corporate records. Each becomes a series of Tool grains (the ingestion process) and Fact grains (extracted knowledge). All carry `structural_tags: ["legal:litigation_hold"]` and `invalidation_policy: {mode: "locked", scope: "lineage"}`.
...
**3. Deposition preparation.** Deposition summaries enter as Events. Key testimony is extracted as Facts with `derived_from` pointing to the Event. Contradictions between testimony and documentary evidence are tracked through `related_to` cross-links with `relation_type: "contradicts"` (Section 14.3).
