...
Consider a consolidated Fact grain that was produced by analyzing three Event grains and extracting a common pattern. The `derived_from` field lists the three episode hashes. The `provenance_chain` provides the detail:
