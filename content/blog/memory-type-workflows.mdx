...
<Callout type="info">
The `steps` and `trigger` fields use their full names as short keys in the compacted form (Section 6.4). Like the Event-specific fields `content` and `consolidated`, the Workflow-specific fields are not shortened further during compaction.
</Callout>
...
The Action memory type (Section 8.5) records every tool invocation an agent makes: the tool name, arguments, result, success/failure, and duration. Over time, these records reveal patterns:
...
3. **Provenance**: The new Workflow carries `derived_from` pointing to the Action grain content addresses that informed the pattern, and `provenance_chain` entries recording the extraction method.
...
Understanding the distinction between Workflows and Beliefs is crucial for correct use of OMS memory types:

| Dimension | Workflow | Belief |
|-----------|----------|--------|
...
| Type byte | `0x04` | `0x01` |
| Required fields | 4 (`type`, `steps`, `trigger`, `created_at`) | 6 (`type`, `subject`, `relation`, `object`, `confidence`, `created_at`) |
| Has `confidence` | No | Yes (required, [0.0, 1.0]) |
| Has `steps` | Yes (required) | No |

The absence of a `confidence` field on Workflows is notable. Beliefs require a confidence score because they are claims that may or may not be true. Workflows are procedures — they either work or they do not. Their effectiveness is tracked through other mechanisms: the `success_count` and `failure_count` fields (available on all grains via the core field map, Section 6.1) and through supersession when an improved workflow replaces an underperforming one.

<Callout type="info">
Workflows and Beliefs serve complementary roles. A Belief might record `subject="disk_cleanup", relation="best_practice", object="archive before delete"`. A Workflow encodes the actual procedure: `trigger="disk usage exceeds 90%", steps=["archive old logs", "delete archived files", "verify space freed"]`. The Belief says what is true; the Workflow says what to do about it.
</Callout>
...
Workflow steps are natural-language descriptions, not executable code. In a trading context, the agent must interpret each step and translate it into actual API calls (which would be recorded as Action grains, Section 8.5). The Workflow captures the strategy; the Actions capture the execution.
</Callout>
...
1. **Learnability**: Agents can extract Workflows from observed patterns in Action and Event grains, formalizing ad-hoc behavior into reusable procedures.
