...
The Tool memory type (Section 8.5) records every tool invocation an agent makes: the tool name, arguments, result, success/failure, and duration. Over time, these records reveal patterns:
...
Workflow steps are natural-language descriptions, not executable code. In a trading context, the agent must interpret each step and translate it into actual API calls (which would be recorded as Tool grains, Section 8.5). The Workflow captures the strategy; the Tools capture the execution.
</Callout>
...
1. **Learnability**: Agents can extract Workflows from observed patterns in Tool and Event grains, formalizing ad-hoc behavior into reusable procedures.
