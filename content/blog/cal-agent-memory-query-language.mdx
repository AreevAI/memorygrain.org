...
<context intent="helping priya prepare the Q2 product roadmap">
  ...
  <fact subject="priya" confidence="0.94">prefers written summaries over verbal briefings</fact>
  <fact subject="priya" confidence="0.91">works in two-week sprint cycles aligned to the first and third Monday</fact>
  <fact subject="priya" confidence="0.87">uses OKR framework for goal-setting; expects outcomes not outputs</fact>
  ...
  <tool tool="search_interviews" phase="completed">retrieved 23 enterprise customer interviews from Q1 2026</tool>
  <tool tool="query_crm" phase="completed">pulled ARR data for 23 accounts; median ARR $180k</tool>
...
CAL/1 ASSEMBLE roadmap_context
  ...
  FROM
    profile:    (RECALL facts ABOUT "priya"
    ...
    actions:    (RECALL tools ABOUT "priya"
                  WHERE tool_phase = "completed" RECENT 5),
...
CAL/1 ASSEMBLE roadmap_context
  FOR "helping priya prepare the Q2 product roadmap"
  FROM
    profile:   (RECALL facts ABOUT "priya" WHERE relation IS PREFERENCE
                 | ORDER BY confidence DESC | LIMIT 10),
...
Facts carry calibrated confidence. Goals have deadlines.
...
ASSEMBLE team_context
  ...
  FROM
    prefs: (RECALL facts WHERE subject IN ($team)
...
```sql
RECALL facts ABOUT "alice" LIMIT 20 AS toon
