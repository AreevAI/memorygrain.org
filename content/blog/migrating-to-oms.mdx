...
description: "A step-by-step guide for migrating existing agent memory systems to the Open Memory Specification — mapping proprietary stores to OMS grain types, defining a namespace strategy, adopting conformance levels incrementally, and using the .mg file as the universal export format."
date: "2026-02-19"
...
### Vector DB entries become Fact grains with embedding_refs

Vector database entries typically combine a text chunk or metadata document with an embedding vector stored for similarity search. In OMS, the knowledge becomes a Fact grain, and the embedding becomes an `embedding_refs` entry.
...
### Key-value stores become Fact grains

Key-value pairs map naturally to the Fact type's semantic triple model: `subject` + `relation` form the key, and `object` is the value.
...
Episodes are intentionally unstructured. They are the input to consolidation --- the process by which an agent extracts structured Facts from raw interactions. The optional `consolidated` boolean tracks whether an episode has been processed. You can migrate entire conversation histories as Event grains and then run consolidation to produce Facts, preserving the full provenance chain via `derived_from`.

### API call logs become Tool grains

Every tool invocation your agent makes --- API calls, function calls, database queries --- maps to a Tool grain. Section 8.5 captures the complete audit record:
...
Using the mappings described above, assign each data source to one or more OMS types. A single source might produce multiple grain types --- a conversation log produces Event grains directly and, after consolidation, Fact grains indirectly.
...
The OMS specification is published under CC0 1.0 Universal (Public Domain Dedication). Anyone can implement it without royalties or restrictions. The format is the interchange layer; how you store, index, query, and serve grains is entirely your choice.
