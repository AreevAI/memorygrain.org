...
A State grain (type byte `0x03`) captures the vehicle's complete operational state at a point in time. This is the AV equivalent of a game save — if the planning system crashes or needs to recover, it restores from the last State grain.
...
## Tools for actuator commands

Every physical action the vehicle takes — steering, braking, acceleration — is an actuator command that can be recorded as a Tool grain (type byte `0x05`):
...
The onboard computer handles sensor fusion, planning, and decision-making. It processes Observation grains from edge sensors, creates Tool grains for actuator commands, and maintains State grains for state recovery. At 32 KB, grains can carry full content references, provenance chains, and cross-links.
...
The type byte at offset 2 enables O(1) filtering: a router can separate Observation grains from Tool grains, Goal grains from State grains, by reading a single byte. No MessagePack deserialization required. In an AV processing pipeline that handles thousands of grains per second, this matters.
...
2. **Tool grains** for the braking command — what deceleration was requested, what was achieved, and the content address of the triggering Observation
3. **State grains** before and after the event — the vehicle's complete state, route, and speed
...
2. **Onboard compute** fuses sensor data, creates Standard-profile Tool grains for actuator commands, maintains State grains for state recovery, and enforces locked Goal grains for safety constraints
