...
The `user_id` field ties this Event to the specific customer for GDPR scoping (Section 12.4). The `namespace` partitions it into the billing support domain. The raw transcript is preserved exactly as captured — no interpretation, no summarization, just faithful recording.
...
Account-level information uses the same Belief structure but adds temporal validity for accuracy over time:
...
When the next agent picks up this case, the State grain provides the complete state: what has been done, what is pending, and what the plan is. The `history` field (optional, array of maps) can record the sequence of actions taken across sessions.
...
A consolidation level 0 Belief is directly extracted from a single Event. A level 1 Belief emerges from frequency analysis across multiple Events. Level 2 identifies co-occurrence patterns. Level 3 detects temporal sequences.

Each consolidated Belief carries `derived_from` (an array of content addresses pointing to source Events) and `source_type: "consolidated"`, creating a complete provenance chain from structured knowledge back to raw interaction data.
...
A customer's complete service memory — every Event, Belief, Goal, Action, State, and Workflow — can be exported as a single .mg file. The file structure (Section 11.2) includes:
...
**Selective disclosure** (Section 10) enables sharing customer insights across teams without exposing PII. A billing team might share a Belief about a customer's payment pattern with the product team, eliding the `user_id` and `subject` fields:
...
The `success_count` and `failure_count` fields on Belief grains (Section 6.1) track which pieces of customer knowledge actually help resolve issues. When an agent uses a Belief — say, that a customer prefers email over phone — and the interaction goes well, `success_count` increments. When a Belief leads to a poor outcome, `failure_count` increments.

Over time, this creates a feedback loop. Beliefs with high success counts become more reliable. Beliefs with rising failure counts signal outdated or inaccurate knowledge that needs reverification. Combined with the `confidence` field (which represents the initial credibility of the knowledge claim), `success_count` and `failure_count` provide empirical validation of customer knowledge.

For example, a Belief recording that a customer prefers Spanish might start with `confidence: 0.95` (high, because the customer explicitly stated it) and accumulate `success_count: 12` over subsequent interactions where routing to Spanish-speaking agents produced positive outcomes. If the customer later switches language preferences and the agent does not catch the update, `failure_count` would start rising — a signal that the knowledge needs refreshing.

## Putting it all together

Consider the full lifecycle for the billing discrepancy scenario:

1. **Capture**: The phone call is recorded as an Event with `user_id: "customer-456"` and `namespace: "support:billing"`.

2. **Extract**: Consolidation produces Beliefs — the customer's subscription tier, their 6-year tenure, the specific charge they dispute.
...
5. **Snapshot**: A State grain captures the current state for handoff to the next session.
...
7. **Resolve**: The Goal transitions to `"satisfied"` with `satisfaction_evidence` pointing to the Action that applied the credit and the Event where the customer confirmed resolution.
