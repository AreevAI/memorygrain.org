...
Events are the input to consolidation — the process of extracting structured Beliefs from unstructured text. A clinical AI agent might process this Event and generate Belief grains: `(patient-456, has_symptom, persistent_cough)`, `(patient-456, prescribed, codeine_cough_suppressant)`. The extracted Beliefs would carry `source_type: "llm_generated"` and reference the Event in their `derived_from` field.
...
- **Complete** — every Belief, Event, Observation, Goal, Action, and State in one file
