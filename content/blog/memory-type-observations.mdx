...
An Observation grain is a **measurement or percept from any kind of observer** — a physical sensor, an AI cognitive agent, or a human. It is designed for high-volume, time-critical data with spatial context. Where a Belief grain records structured knowledge (`"server-room" "has_temperature" "22.5C"` with confidence 0.95), an Observation grain records the raw measurement event: which observer, what type, what was observed, when, and in what coordinate frame.

The distinction matters. Beliefs are knowledge claims with confidence scores and temporal validity windows. Observations are measurement events with observer identity and spatial metadata. A system might generate thousands of Observation grains per second from a LiDAR scanner, then consolidate them into a handful of Belief grains that describe the environment. An AI agent might record cognitive observations about patterns it detects in incoming data. The two types serve different stages of the data pipeline.
...
The Observation type has a default `importance` of 0.3. This is notably lower than the Belief type's default of 0.7. The reasoning is straightforward: observations are high-volume, and most are routine.
...
- **Belief derivation.** A consolidation engine might process thousands of temperature Observation grains and produce a single Belief grain: `"server-room" "average_temperature" "22.3C"` with `derived_from` pointing to the source observations.
...
- **Event context.** An Event grain recording a maintenance event can reference Observation grains via `related_to` cross-links, connecting the narrative record to the raw sensor data that triggered the event.
- **Action inputs.** When an agent invokes a sensor processing tool, the Action grain's `content_refs` might point to the same raw data as the Observation grain, creating a link between what was observed and what was done with the observation.
