...
- **Tool inputs.** When an agent invokes a sensor processing tool, the Tool grain's `content_refs` might point to the same raw data as the Observation grain, creating a link between what was observed and what was done with the observation.
