...
If the trading agent crashes or the session needs to be reconstructed, the State grain provides a complete snapshot. The `context` map captures open positions, P&L, buying power, margin utilization, trade count, and risk exposure — everything needed to resume operations or audit the session state at a specific point in time.
...
Like clinical Events in healthcare, these are the input to consolidation — the process of extracting structured Beliefs. The trading agent might process this Event and generate Belief grains: `(AAPL, earnings_surprise, +12%)`, `(AAPL, revenue_growth, 18% YoY)`. The extracted Beliefs carry `source_type: "llm_generated"` and reference the Event in their `derived_from` field.
...
Every grain carries a `provenance_chain` — the full derivation trail showing where the grain came from and how it was produced. For a trading decision, this chain might trace from raw market data (Event) through signal generation (Belief) through strategy trigger (Workflow reference) to order execution (Action).
...
A portfolio performance Belief shared with a client might elide the `provenance_chain` (which reveals the firm's analytical methods) and `namespace` (which reveals organizational structure):
...
1. **Market data ingestion**: Raw analyst reports and earnings transcripts become Event grains. Market reference data becomes Belief grains with `source_type: "imported"`.

2. **Signal generation**: The trading agent analyzes Episodes and market Facts, generating signal Belief grains with `source_type: "llm_generated"`, temporal validity windows (`valid_from`/`valid_to`), and `provenance_chain` tracing the analysis inputs.
...
5. **Order execution**: The agent submits an order via the broker API, creating a Action grain with complete arguments, result, fill price, and execution time.

6. **Session state**: At regular intervals and after significant events, State grains capture the complete trading session state — positions, P&L, margin, risk exposure.

7. **Regulatory audit**: Years later, a regulator asks what the agent knew when it made a specific trade. The bi-temporal query reconstructs the exact system state at the moment of the decision. The provenance chain traces from the trade (Action) back through the strategy (Workflow), the signals (Beliefs), and the raw data (Events). Every grain is content-addressed, COSE-signed, and immutable.
