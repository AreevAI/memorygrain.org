...
A customer success agent is helping Elena with her account renewal. The memory store has three facts, one goal, and one tool result. Here is the same data rendered four ways.
...
<context intent="helping elena with account renewal negotiation">
  ...
  <fact subject="elena" confidence="0.93">prefers multi-year terms when pricing is fixed</fact>
  <fact subject="elena" confidence="0.88">has budget authority up to $500k annually</fact>
  <fact subject="elena" confidence="0.81">makes renewal decisions in Q1</fact>

  <goal subject="elena" state="active" deadline="2026-03-31">negotiate renewal before fiscal year close</goal>

  <tool tool="get_renewal_quote" phase="completed">450 seats at $290/seat = $1.44M/yr; 3-year at $265/seat; 18% discount for 3-year commit</tool>
...
```
facts[3]{subject,confidence,content}:
...
```json
{
  ...
  "grains": [
    {"type": "fact", "subject": "elena", "confidence": 0.93,
     "content": "prefers multi-year terms when pricing is fixed"},
    {"type": "fact", "subject": "elena", "confidence": 0.88,
     "content": "has budget authority up to $500k annually"},
    {"type": "fact", "subject": "elena", "confidence": 0.81,
    ...
    {"type": "tool", "tool": "get_renewal_quote",
     "phase": "completed",
...
TOON achieves its savings through tabular layout: the grain type and column names appear once per section, not once per grain. For uniform arrays (50 facts about the same user), the savings compound. For mixed-type assemblies, TOON groups by type and each group gets its own header — still compact, but the per-section overhead adds up.

## When to Use Each

### SML — Default for LLM System Prompts

Use SML when the model needs to distinguish between types of information. The tag name is the signal: `<fact confidence="0.65">` tells the model to hedge. `<consent action="denied">` tells it what it cannot do. `<reasoning type="abductive">` tells it an inference has already been drawn.

```sql
ASSEMBLE user_context
  FOR "resolving elena's renewal"
  FROM facts: (RECALL facts ABOUT "elena" LIMIT 10),
...
```sql
RECALL facts ABOUT "elena"
  WHERE relation IS PREFERENCE
  | ORDER BY confidence DESC | LIMIT 50
  AS toon
```

50 facts in TOON uses roughly the same tokens as 30 facts in JSON. If you are building a knowledge retrieval step that feeds a second LLM call, TOON preserves more information within the same budget.
...
ASSEMBLE escalation_summary
  ...
  FROM history: (RECALL events ABOUT "elena" RECENT 20),
       actions: (RECALL tools ABOUT "elena" RECENT 10)
  BUDGET 4000 tokens
  FORMAT markdown
```
...
ASSEMBLE account_snapshot
  ...
  FROM facts: (RECALL facts ABOUT "elena"),
...
CAL/1 ASSEMBLE mixed_context
  ...
  FROM
    profile: (RECALL facts ABOUT "elena"
               WHERE relation IS PREFERENCE | LIMIT 10 AS toon),
    history: (RECALL events ABOUT "elena" RECENT 10 AS sml),
    actions: (RECALL tools ABOUT "elena" RECENT 5 AS markdown)
  BUDGET 4000 tokens
  PRIORITY history > profile > actions
  FORMAT sml
```

Profile data comes as compact TOON — saves tokens on a uniform array of facts. Conversation history comes as SML — preserves epistemic signals for the model. Tools come as Markdown — readable in an audit trail. Each source is optimised independently.
