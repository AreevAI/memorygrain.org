...
4. **Support all 10 grain types** — Fact, Event, State, Workflow, Tool, Observation, Goal, Reasoning, Consensus, and Consent. Your deserializer must handle the required fields for each.
...
- **Validate required fields per schema** — A Fact must have `type`, `subject`, `relation`, `object`, `confidence`, and `created_at`. Reject grains with missing required fields.
...
Byte 2:    Type (memory type enum: 0x01=Fact, 0x02=Event, ..., 0x0A=Consent)
...
After decompaction, check that the `type` field exists. If absent, reject with `ERR_NO_TYPE`. The value must be one of: `"fact"`, `"event"`, `"state"`, `"workflow"`, `"action"`, `"observation"`, or `"goal"`.
...
**Input (a minimal Fact grain, using the v1.0 type name `"fact"`; v1.2 canonical name is `"fact"`):**
