...
4. **Support all 10 grain types** — Belief, Event, State, Workflow, Action, Observation, Goal, Reasoning, Consensus, and Consent. Your deserializer must handle the required fields for each.
...
- **Validate required fields per schema** — A Belief must have `type`, `subject`, `relation`, `object`, `confidence`, and `created_at`. Reject grains with missing required fields.
...
Byte 2:    Type (memory type enum: 0x01=Belief, 0x02=Event, ..., 0x0A=Consent)
