...
SML (Semantic Markup Language) is a flat, tag-based format where the tag name itself is the signal. It uses the 10 OMS grain type names — `fact`, `goal`, `event`, `action`, `observation`, `reasoning`, `state`, `workflow`, `consensus`, `consent` — as element names. Each name tells the LLM what epistemic category the content belongs to before it reads a single word.

```sml
<fact subject="alice" confidence="0.94">prefers dark mode in all tools</fact>
<fact subject="alice" confidence="0.61">probably prefers dark mode</fact>
```

The model now sees two facts about dark mode — one at 94% confidence, one at 61%. It can weight them differently. It can mention the stronger fact as fact and the weaker one as a possibility. No prompt instruction needed. The structure communicates the distinction.
...
### `<fact>` — What the System Knows (With Calibrated Certainty)

Facts are semantic triples with a confidence score. They represent structured knowledge — user preferences, relationships between entities, operational facts.

```sml
<fact subject="marcus" confidence="0.96">enterprise plan customer since 2023-01</fact>
<fact subject="marcus" confidence="0.89">consistently pays on net-30 terms; no prior late payments</fact>
<fact subject="marcus" confidence="0.74">tends to escalate billing issues directly to VP of Customer Success</fact>
<fact subject="server-room-3" confidence="0.98">temperature must stay between 18C and 24C</fact>
<fact subject="deployment-v4.2" confidence="0.82">caused 12% latency increase in checkout flow</fact>
```

The confidence score is not decorative. When an LLM sees `0.74`, it should treat that fact as likely but unconfirmed. When it sees `0.98`, it should treat it as a hard constraint. The tag name (`fact`) plus the attribute (`confidence`) create a two-dimensional signal that plain text cannot carry.
...
### `<tool>` — What Tools Have Already Been Called

Tools record tool invocations and their results. The `phase` attribute signals whether the action is pending, completed, or errored.

```sml
<tool tool="get_invoice" phase="completed">INV-2024-8831: $4,200 for 28 seats at $150/seat</tool>
<tool tool="get_contract" phase="completed">contract shows 24 seats at $150/seat = $3,600/month</tool>
<tool tool="get_seat_history" phase="completed">4 seats added 2026-02-14; amendment not yet executed</tool>
<tool tool="send_amendment" phase="error">failed — legal review required for mid-term amendments</tool>
```
...
Observations carry lower implied certainty than explicit facts. They are things the system perceived without the user stating them.
...
Consensus grains carry more epistemic weight than single facts. They represent claims independently verified by multiple observers.
...
<fact subject="marcus">enterprise plan customer since 2023-01</fact>

<!-- standard: adds confidence -->
<fact subject="marcus" confidence="0.96">enterprise plan customer since 2023-01</fact>

<!-- full: adds source, observation time, provenance -->
<fact subject="marcus" confidence="0.96" source="crm-sync" observed="1d ago">enterprise plan customer since 2023-01</fact>
```
...
<context intent="resolving marcus's billing dispute for invoice INV-2024-8831">
  ...
  <fact subject="marcus" confidence="0.96">enterprise plan customer since 2023-01</fact>
  <fact subject="marcus" confidence="0.89">consistently pays on net-30; no prior late payments</fact>
  <fact subject="marcus" confidence="0.82">primary contact for billing decisions</fact>
  ...
  <tool tool="get_invoice" phase="completed">INV-2024-8831: $4,200 for 28 seats at $150/seat</tool>
  <tool tool="get_contract" phase="completed">contract: 24 seats at $150/seat = $3,600/month; renewal 2026-04-01</tool>
  <tool tool="get_seat_history" phase="completed">4 seats added 2026-02-14; amendment not executed</tool>
...
The tag names do the heavy lifting. The model knows what to trust (`consensus`), what is certain (`fact` at 0.96), what was inferred (`reasoning`), and what it is authorised to access (`consent`). This is not prompt engineering. It is structural communication.
