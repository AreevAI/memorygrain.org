...
A diagnostic AI creates Belief grains linking patients to conditions. Each Belief references the medical images (X-rays, MRIs, CT scans) that informed the diagnosis via `content_refs`. The images themselves live in a PACS (Picture Archiving and Communication System) or content-addressed store.
