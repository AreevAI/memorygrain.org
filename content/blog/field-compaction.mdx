...
|-----------|-----------|------|-------------|
| `type` | `t` | string | Grain type: "fact", "event", "state", "action", etc. |
...
| `tool_phase` | `aphase` | string | v1.2 new: `"definition"` \| `"call"` \| `"result"` |
...
Tool fields see meaningful compaction. `tool_name` (9 bytes) becomes `tn` (2 bytes). `input` (5 bytes) becomes `inp` (3 bytes). Note: the old `arguments`/`args`, `result`/`res`, and `success`/`ok` short keys were **removed** in v1.2 — implementations emitting those keys are non-conformant.
...
<Callout type="info">
**OMS v1.3 update:** The Integration Domain Profile (Appendix A.7) adds 25 new compact keys for action and trigger fields in the `int:` namespace, including keys for `output_schema`, trigger conditions, and action pipeline metadata. Five compact key collisions from v1.2 were also resolved. See the [OMS v1.3 specification](https://github.com/openmemoryspec/oms) for the complete updated field mapping table (Appendix C).
</Callout>
