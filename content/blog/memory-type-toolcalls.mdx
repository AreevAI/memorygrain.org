...
<Callout type="info">
**New in OMS v1.3:** The `output_schema` field allows Action definition grains (`action_phase: "definition"`) to declare a JSON Schema (draft-07) describing the action's return value. This enables consumers to validate action results structurally and supports the new Integration Domain Profile's trigger/action pipeline.
</Callout>
...
For the complete Action schema, field compaction keys, and serialization rules, see [Section 8.5 of the OMS v1.3 specification](https://github.com/openmemoryspec/oms).
